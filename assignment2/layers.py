...
class ReLULayer:
    ...
    def forward(self, X):
        ...
        self.cache = np.greater(X, 0).astype(float)  # dZ/dx
        out = np.maximum(X, 0)  # Z
        return out
    ...
    def backward(self, d_out):
        ...
        d_result = np.multiply(d_out, self.cache)  # dL/dX = dL/dZ * dZ/dX
        return d_result
...
class FullyConnectedLayer:
    ...
    def backward(self, d_out):
        ...
        d_result = np.dot(d_out, self.W.value.T)  # dL/dX = dL/dZ * W.T
        dLdW = np.dot(self.X.T, d_out)  # dL/dW = X.T * dL/dZ
